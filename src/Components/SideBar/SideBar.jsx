...
export default function SideBar() {
    const{showMenu,setShowMenu}=useContext(tokenContext)
  return (
  <>
  <div className={`md:hidden dock dock-md fixed bottom-0 ${ showMenu ? "blur":"" }`}>
  <Link to='' end='true' >
    <button>
  ...
  <button onClick={()=>{setShowMenu(true)}} className="p-5 bg-main rounded-full h-[50px] w-[50px]">
 ...
 <nav className={`hidden md:flex md:flex-col fixed start-0 top-0 bottom-0 p-5  w-[20%] shadow ${ showMenu ? "blur":"" }`}>
