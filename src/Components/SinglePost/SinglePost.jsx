...
export default function SinglePost() {
  ...
  function getSinglePost(pid) {
    axios
      .get(`https://linked-posts.routemisr.com/posts/${pid}`, {
        headers: {
      ...
      {isPosted ? (
        <div className="toast toast-top toast-center mt-12">
          <div className="alert alert-success">
            <span className="poppins">Comment sent successfully.</span>
          </div>
        </div>
      ) : isError.status ? (
        <div className="toast toast-top toast-center mt-12">
          <div className="alert alert-error">
            <span className="poppins">{isError.msg}</span>
          </div>
        </div>
      ) : (
        ""
              ...
              <div className="flex items-center justify-between gap-4">
                <div className="flex gap-4">
                  ...
                  <div className="flex flex-col">
                    <h2 className="poppins font-bold">{post.user.name}</h2>{" "}
                    {/* Name */}
                    <div className="poppins mt-0 text-slate-400 text-sm flex gap-1">
                      <span>@{post.user.name.split(" ").pop()} .</span>
                      <span> {shortFormat(post.createdAt)}</span>
                    </div>{" "}
                    {/* @name*/}
                  </div>
                </div>
